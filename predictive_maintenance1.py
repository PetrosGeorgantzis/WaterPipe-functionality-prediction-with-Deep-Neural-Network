...
class_Array = []              ################################## ARRAY OF CLASSES OF PREDICTIONS ###############################################
class_Array_train = []
class_Array_test = []
	...
	with open(filename) as csvfile:
		...
		for row in reader:
			if (row[0].isdigit()):
				...
				float(row[13]),float(row[14]),str(row[15]),str(row[16]),float(row[17]),str(row[18]),str(row[20]),str(row[22]),float(row[23]),str(row[24]),str(row[25]),
				str(row[26]),str(row[27]),str(row[28]),str(row[29]),str(row[31]),str(row[32]),str(row[33]),str(row[35]),str(row[36]),str(row[37]),str(row[38]),
				str(row[39]),str(row[21])])			
				#################################################################################################################################################################
	...
	while (i < len(Array)):
		...
		while (j < length):
			...
			if (maximum < Array[i][j]):
				maximum = Array[i][j]		########################### FIND THE MAXIMUM SO WE CAN NORMALIZE ######################################
			if (minimum > Array[i][j]):
				minimum = Array[i][j]		########################### FIND THE MINIMUM SO WE CAN NORMALIZE ######################################	
...
def NormalizeData(Array):
	...
	while (i < len(Array)):
		j = 0				###################################### COUNTER VARIABLE TO TRACK THE ELEMENT INSIDE THE ARRAY OF THE DATASET ARRAY #########################
		while (j < length):
			Array[i][j] = (Array[i][j] - minimum)/(maximum-minimum)
			j += 1
		i+= 1
...
def RandomGenerateSamples(length):
	...
	while (i < length):
		...
		if ((train_counter < 55000) and (test_counter <4400)):
			rand = random.randint(0,1)
			if (rand == 1):
				Dataset_train.append(Dataset[i])
				Predictions_train.append(Predictions[i])
				class_Array_train.append(class_Array[i])
				train_counter += 1
			else:
				Dataset_test.append(Dataset[i])
				Predictions_test.append(Predictions[i])
				class_Array_test.append(class_Array[i])
				test_counter += 1
		else:
			if (train_counter >= 55000):
				Dataset_test.append(Dataset[i])
				Predictions_test.append(Predictions[i])
				class_Array_test.append(class_Array[i])
				test_counter += 1
...
def NeuralNetworkTrain(Array1,Array2):
	...
	clf = MLPClassifier(hidden_layer_sizes=(1000),activation='relu',learning_rate_init=0.001,max_iter = 200).fit(X_train,Y_train)
...
def main():
	...
	print("waterpoint_type_group: ", waterpoint_type_Array, "\n")
