...
def impute_strings_by_nearby_samples(samples_df: pd.DataFrame,location_column: str,target_column: str,strings,frequency_threshold: float or None) -> pd.DataFrame:
	for area in samples_df[location_column].unique():
		row_ids = samples_df[location_column] == area
		target_values = samples_df.loc[row_ids, target_column]
...
def impute_strings_by_simillar_strings(samples_df : pd.DataFrame,location_column: str,similarity_ratio: float or None, frequency_threshold: int)-> pd.DataFrame:
	Change_Array = []
	Array = []
	Array_of_uniques = sorted(samples_df[location_column].unique())
	for z in range(len(Array_of_uniques)+1):
		if z == len(Array_of_uniques):
			Array.append(['Other',z])
		else:
			Array.append([Array_of_uniques[z],z])
	...
	while i < len(Array_of_uniques)-1:
		...
		if (len(Change_Array)> 0):
			for length in Change_Array:
				samples_df[location_column] = samples_df[location_column].replace({str(length):str(Array_of_uniques[i])})
				...
				print(f'I changed {length} with {Array_of_uniques[i]}')
			Change_Array.clear()
	...
	for change in samples_df[location_column].unique():
		...
		if (target_values.count() <  frequency_threshold):
			...
			for z in range(len(Array)-1):
				if Array[z][0] == change:
					Array[z][1] = len(Array) - 1

	return samples_df,Array


#################################################### FUNCTION TO LOAD THE DATASET FROM THE FILE ####################################################################################
####################################################################################################################################################################################

def impute_test_strings(samples_df : pd.DataFrame,tests_df : pd.DataFrame,location_column: str,similarity_ratio: float or None,frequency_threshold: int)-> pd.DataFrame:
	Change_Array = []

	Array_of_uniques_samples = samples_df[location_column].unique().tolist()
	Array_of_uniques_tests = tests_df[location_column].unique().tolist()
	i = 0
	j = 0
	while i < len(Array_of_uniques_tests):
		while j < len(Array_of_uniques_samples):
			s = SequenceMatcher(None,(str(Array_of_uniques_tests[i].lower())),(str(Array_of_uniques_samples[j].lower()))).ratio()
			if (s > similarity_ratio):
				print(f'Ratio is: {s} between {Array_of_uniques_samples[j]} and {Array_of_uniques_tests[i]}')
				Change_Array.append(Array_of_uniques_samples[j])
			j += 1
		if (len(Change_Array)> 0):
			for length in Change_Array:
				samples_df[location_column] = samples_df[location_column].replace({str(length):str(Array_of_uniques_tests[i])})
				Array_of_uniques_samples.remove(length)
				print(f'I changed {length} with {Array_of_uniques_tests[i]}')
			Change_Array.clear()
		print(f'Length of unique {location_column} is: ', len(Array_of_uniques_samples))
		print(f'I am in {i+1} loop')
		i += 1
		j = 0
	Array_of_uniques_samples.clear()
	Array_of_uniques_tests.clear()
	for change in samples_df[location_column].unique():
		row_ids = samples_df[location_column] == change
		target_values = samples_df.loc[row_ids, location_column]
		if (target_values.count() <  frequency_threshold):
...
def improve_values(samples,tests,flag):
	...
	samples,funder_Array = impute_strings_by_simillar_strings(samples_df = samples,location_column = 'funder',
	                                   similarity_ratio = similarity_ratio,
	                                   frequency_threshold = frequency_threshold)

	if (flag == False):
		samples = impute_test_strings(samples_df = samples,tests_df = tests ,location_column = 'funder',
	...
	samples,installer_Array = impute_strings_by_simillar_strings(samples_df = samples,location_column = 'installer',
	                                   similarity_ratio = similarity_ratio,
	                                   frequency_threshold = frequency_threshold)

	if (flag == False):
		samples = impute_test_strings(samples_df = samples,tests_df = tests ,location_column = 'installer',
		                              similarity_ratio = 0.65,
		                              frequency_threshold = frequency_threshold)
	...
	samples,scheme_name_Array = impute_strings_by_simillar_strings(samples_df = samples,location_column = 'scheme_name',
	                                   similarity_ratio = similarity_ratio,
	                                   frequency_threshold = frequency_threshold)

	if (flag == False):
		samples = impute_test_strings(samples_df = samples,tests_df = tests ,location_column = 'scheme_name',
		                              similarity_ratio = 0.65,
		                              frequency_threshold = frequency_threshold)
	...
	return samples,funder_Array,installer_Array,scheme_name_Array


#################################################### FUNCTION TO MAKE STRINGS TO CLASSES AS NUMBERS FOR INPUTS IN OUR NEURAL NETWORK #######################################################
##############################################################################################################################################################################################

def make_dataset(samples,funder_Array,installer_Array,scheme_name_Array):
		...
		if (areas == 'permit' or areas == 'public_meeting'):
			samples[areas] = samples[areas].replace({'False': 0,'True': 1, 'unknown': -1})
				...
				for values in Array_of_uniques:
					for i in range(len(funder_Array)):
						if values == funder_Array[i][0]:
							samples[areas] = samples[areas].replace({values:funder_Array[i][1]})

		elif (areas == 'installer'):
				Array_of_uniques = sorted(samples[areas].unique())
				for values in Array_of_uniques:
					for i in range(len(installer_Array)):
						if values == installer_Array[i][0]:
							samples[areas] = samples[areas].replace({values:installer_Array[i][1]})

		elif (areas == 'scheme_name'):
				Array_of_uniques = sorted(samples[areas].unique())
				for values in Array_of_uniques:
					for i in range(len(scheme_name_Array)):
						if values == scheme_name_Array[i][0]:
							samples[areas] = samples[areas].replace({values:scheme_name_Array[i][1]})
		else:
				Array_of_uniques = sorted(samples[areas].unique())
				for values in Array_of_uniques:
...
def neural_network_train(Dataset,Predictions):
	...
	x0 = tf.keras.layers.Dense(256, activation=tf.math.sigmoid)(inputs)
	y0 = tf.keras.layers.Dense(128, activation=tf.math.sigmoid)(x0)
	x1 = tf.keras.layers.Dense(64, activation=tf.math.sigmoid)(y0)
	y1 = tf.keras.layers.Dense(32, activation=tf.math.sigmoid)(x1)
	x2 = tf.keras.layers.Dense(64, activation=tf.math.sigmoid)(y1)
	y2 = tf.keras.layers.Dense(128, activation=tf.math.sigmoid)(x2)
	x3 = tf.keras.layers.Dense(256, activation=tf.math.sigmoid)(y2)
	outputs = tf.keras.layers.Dense(3, activation=tf.nn.softmax)(x3)
...
def main():
	...
	if(os.path.exists('train.csv') == False):
		...
		samples,funder_Array,installer_Array,scheme_name_Array = improve_values(samples,samples,True)
	...
	if (os.path.exists('train_number.csv') == False):
		...
		samples = make_dataset(samples,funder_Array,installer_Array,scheme_name_Array)
	...
	if (os.path.exists('testing.csv') == False):
		...
		tests,Array1,Array2,Array3 = improve_values(tests,samples,False)
	...
	if(os.path.exists('testing_numbers.csv') == False):
		...
		tests = make_dataset(tests,funder_Array,installer_Array,scheme_name_Array)
